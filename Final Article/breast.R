...
brest.data <- read.delim("C:/Users/gvamorim/Documents/Personal/Repositories/ANN-Artificial-Neural-Network/Final Article/breast-cancer-wisconsin.data",header=FALSE, sep=",", na.strings = '?')
brest.data <- brest.data[complete.cases(brest.data),]
data_formated<-data.matrix(brest.data, rownames.force = NA)

X<-as.matrix(data_formated[1:dim(data_formated)[1], 2:10])
Y<-as.matrix(data_formated[1:dim(data_formated)[1], 11])
for(i in 1:length(Y)){
  if(Y[i] == 2){
    Y[i]<-0
  } else {
